...
public class EPLUtil {
    ...
    public static EPDeployment deploy(EPRuntime runtime, EPCompiled compiled) {
        return deploy(runtime, compiled, UUID.randomUUID().toString());
    }

    public static EPDeployment deploy(EPRuntime runtime, EPCompiled compiled, String deploymentId) {
        EPDeployment deployment;
        try {
            deployment = runtime.getDeploymentService().deploy(compiled, new DeploymentOptions().setDeploymentId(deploymentId));
