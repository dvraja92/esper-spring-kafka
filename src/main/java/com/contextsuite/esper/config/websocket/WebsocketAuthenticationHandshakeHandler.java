...
@Slf4j
public class WebsocketAuthenticationHandshakeHandler implements HandshakeHandler, Lifecycle {
    ...
    public WebsocketAuthenticationHandshakeHandler(){
        this(initRequestUpgradeStrategy());
    }

    public WebsocketAuthenticationHandshakeHandler(Object authenticatorObj, String invokeMethodName) {
        this(initRequestUpgradeStrategy());
        this.authenticatorObj = authenticatorObj;
        this.invokeMethodName = invokeMethodName;
    }

    public WebsocketAuthenticationHandshakeHandler(RequestUpgradeStrategy requestUpgradeStrategy) {
    ...
    public final boolean doHandshake(ServerHttpRequest request, ServerHttpResponse response, WebSocketHandler wsHandler, Map<String, Object> attributes) throws HandshakeFailureException {
        ...
        if (log.isTraceEnabled()) {
            log.trace("Processing request " + request.getURI() + " with headers=" + headers);
        }
        ...
        try {
            ...
            log.error("User must logged in");
            e.printStackTrace();
            return false;
        }

        if (log.isTraceEnabled()) {
            log.trace("Upgrading to WebSocket, subProtocol=" + subProtocol + ", extensions=" + extensions);
        }
    ...
    protected void handleInvalidUpgradeHeader(ServerHttpRequest request, ServerHttpResponse response) throws IOException {
        if (log.isErrorEnabled()) {
            log.error("Handshake failed due to invalid Upgrade header: " + request.getHeaders().getUpgrade());
        }
    ...
    protected void handleInvalidConnectHeader(ServerHttpRequest request, ServerHttpResponse response) throws IOException {
        if (log.isErrorEnabled()) {
            log.error("Handshake failed due to invalid Connection header " + request.getHeaders().getConnection());
        }
    ...
    protected void handleWebSocketVersionNotSupported(ServerHttpRequest request, ServerHttpResponse response) {
        if (log.isErrorEnabled()) {
            ...
            log.error("Handshake failed due to unsupported WebSocket version: " + version + ". Supported versions: " + Arrays.toString(this.getSupportedVersions()));
        }
